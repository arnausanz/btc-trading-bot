...
class RLBot(BaseBot):
    """
    Bot agnòstic a l'agent RL.
    Paral·lel a MLBot però per a agents de reinforcement learning.
    Afegir un agent nou = afegir una entrada a _AGENT_REGISTRY.
    """
    ...
    def _load_agent(self) -> BaseRLAgent:
        ...
        logger.info(f"RLBot carregat: {agent_type} des de {model_path}")
        return agent
    ...
    def on_observation(self, observation: dict) -> Signal:
        ...
        # Construeix l'observació aplanada igual que BtcTradingEnv._get_observation()
    ...
    def on_start(self) -> None:
        logger.info(f"RLBot iniciat amb agent: {self.config['model_type']}")